...
// Call_back function for received messages
int get_data(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data) {
    SharedState* state = static_cast<SharedState*>(user_data);
    state->obj_id = argv[0]->f;
    state->x = argv[1]->f;
    state->z = argv[2]->f;
    state->x_vel = argv[3]->f;
    state->z_vel = argv[4]->f;
    state->abs_vel = argv[5]->f;

    //float x=argv[0]->f;
    //float y=argv[1]->f;
    //float z=argv[2]->f;
    std::cout << "Received message on " << path << ": " <<  state->x  << state->z<< std::endl;
